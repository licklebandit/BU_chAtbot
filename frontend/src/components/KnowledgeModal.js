...
    // Payload uses 'title' and 'content' to match the updated backend router
    const payload = { title, content }; 
    const url = `${API_BASE}${isEdit ? '/' + article._id : ''}`;
...
      setError(`Failed to ${isEdit ? 'update' : 'create'} article. (Error: ${err.response?.status || err.code})`);
    } finally {
      setLoading(false);
    }
  };
